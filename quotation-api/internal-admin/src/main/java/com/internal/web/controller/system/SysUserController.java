...
public class SysUserController extends BaseController
{
    ...
    @PostMapping("/export")
    @Operation(summary = "用户导出", description  = "用户导出")
    ...
    public AjaxResult importData(@RequestPart(name = "file") MultipartFile file,@RequestParam("updateSupport") boolean updateSupport) throws Exception
    {
        ExcelUtil<SysUser> util = new ExcelUtil<SysUser>(SysUser.class);
...
//        ajax.put("roles", SysUser.isAdmin(userId) ? roles : roles.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList()));
     ...
     * 获取指定角色的用户列表
     */
    @Operation(summary = "获取指定角色的用户列表", description  = "获取指定角色的用户列表")
