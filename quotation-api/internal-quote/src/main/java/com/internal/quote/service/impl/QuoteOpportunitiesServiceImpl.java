...
public class QuoteOpportunitiesServiceImpl extends ServiceImpl<QuoteOpportunitiesMapper, QuoteOpportunities> implements IQuoteOpportunitiesService
{
    ...
    public List<QuoteOpportunitiesVO> getQuoteOpportunitiesList(QuoteOpportunitiesQuery model,Boolean selectDetail) {
        ...
        if(selectDetail)
        {
            ...
            Map<String,SysUser> userMapByCrm = userList.stream()
                    .filter(x -> ObjectUtil.isNotEmpty(x.getCrmUserId()))
                    .collect(Collectors.toMap(SysUser::getCrmUserId,x -> x));
            Map<Long,SysUser> userMapById = userList.stream()
                    .filter(x -> ObjectUtil.isNotEmpty(x.getUserId()))
                    .collect(Collectors.toMap(SysUser::getUserId,x -> x));
            ...
            for (QuoteOpportunitiesVO vo: voList) {
                ...
                if(ObjectUtil.isNotNull(vo.getType())){
                    if(vo.getType().equals(QuoteType.COST.getCode())){
                        if(ObjectUtil.isNotEmpty(detail)){
                            ...
                            vo.setQuoteDesc(detail.getValuationDesc());
                        }
                        ...
                        if(ObjectUtil.isNotEmpty(rough)){
                            ...
                            vo.setQuoteDesc(rough.getValuationDesc());
                        }
                    }else {
                ...
                //设置前端判断条件
                quoteUtil.setQuoteMethod(vo,currentUser,userMapByCrm);

                //判断是否是多部门
                 vo.setIsMultiDept(isMultiDept(userMapByCrm,vo.getSupportPerson()));

                //审批人
                SysUser saleApprove = MapUtil.getMapValue(userMapById,vo.getSaleAuditPerson());
                SysUser signApprove = MapUtil.getMapValue(userMapById,vo.getSignAuditPerson());
                SysUser loseApprove = MapUtil.getMapValue(userMapById,vo.getLoseAuditPerson());
                if(ObjectUtil.isNotEmpty(saleApprove)){
                    vo.setSaleAuditPersonName(saleApprove.getNickName());
                }
                if(ObjectUtil.isNotEmpty(signApprove)){
                    vo.setSignAuditPersonName(signApprove.getNickName());
                }
                if(ObjectUtil.isNotEmpty(loseApprove)){
                    vo.setLoseAuditPersonName(loseApprove.getNickName());
                }
            }
        }
    ...
    public Boolean quoteApply(ReQuoteDTO dto) {
        ...
        if(CollUtil.isEmpty(dto.getPreSaleIdList())){
            throw new RuntimeException("售前列表为空");
        }
        SysUser currentUser = SecurityUtils.getLoginUser().getUser();
        ...
        if(updatePerson){
            ...
            QuoteOpportunities newQuote = BeanUtil.toBean(quote,QuoteOpportunities.class);
            newQuote.setId(null);
            ...
            newQuote.setReQuoteDesc(dto.getQuoteDesc());

            //清空审批记录
            newQuote.setSaleAuditLog("");
            newQuote.setSaleAuditPerson(null);
            newQuote.setSaleAuditTime(null);
            newQuote.setSignAuditLog("");
            newQuote.setSignAuditPerson(null);
            newQuote.setSignAuditTime(null);

            newQuote.setLoseType(null);
            newQuote.setLoseAuditDesc("");
            newQuote.setLoseTime(null);
            newQuote.setLoseAuditLog("");
            newQuote.setLoseAuditPerson(null);
            newQuote.setLoseAuditTime(null);
            newQuote.setStatusBeforeLose(null);
            ...
            LambdaUpdateWrapper<QuoteOpportunities> updateWrapper = Wrappers.<QuoteOpportunities>lambdaUpdate()
                    ...
                    //销售审批人，审批时间，审批记录（理由在销售表里）
                    .set(QuoteOpportunities::getSaleAuditPerson,null)
                    .set(QuoteOpportunities::getSaleAuditLog,"")
                    .set(QuoteOpportunities::getSaleAuditTime,null)
                    //签约审批人，审批时间，审批记录（理由在签约表里）
                    .set(QuoteOpportunities::getSignAuditPerson,null)
                    .set(QuoteOpportunities::getSignAuditTime, null)
                    .set(QuoteOpportunities::getSignAuditLog, "")
                    //丢单类型,丢单审批时间,审批记录,审批人,审批理由，丢单时间,丢单理由
                    .set(QuoteOpportunities::getLoseType, null)
                    .set(QuoteOpportunities::getLoseAuditTime, null)
                    .set(QuoteOpportunities::getLoseAuditLog,"")
                    .set(QuoteOpportunities::getLoseAuditPerson,null)
                    .set(QuoteOpportunities::getLoseAuditDesc,"")
                    .set(QuoteOpportunities::getLoseTime,null)
                    .set(QuoteOpportunities::getReasonDesc,null)

                    .set(QuoteOpportunities::getNonQuoters, "")
                    .set(QuoteOpportunities::getReQuoteDesc, dto.getQuoteDesc())
                    ...
                    .set(QuoteOpportunities::getCreateBy, quote.getCreateBy())
                    .set(QuoteOpportunities::getUpdateTime, DateTime.now())
    ...
    @Override
    @DSTransactional()
    public Boolean lose(LoseDTO dto) {
        QuoteOpportunities quote = baseMapper.selectById(dto.getId());
        if(ObjectUtil.isEmpty(quote)){
            return false;
        }

        String loseType = dto.getLoseType();
        String lastStatus = getLastStatusByLoseType(loseType);
        //如果上一个状态是丢单审批未通过
        if(QuoteStatusEnum.LOSE_REJECTED.getCode().equals(quote.getStatus())){
            lastStatus = QuoteStatusEnum.LOSE_REJECTED.getCode();
        }

        QuoteLoseType quoteLoseType = QuoteLoseType.getByCode(loseType);
        if(ObjectUtil.isNull(quoteLoseType)){
            throw new ServiceException("无效的丢单类型.请重新检查", 400018);
        }
        ...
        LambdaUpdateWrapper<QuoteOpportunities> updateWrapper = Wrappers.<QuoteOpportunities>lambdaUpdate()
                ...
                .set(QuoteOpportunities::getLoseType,dto.getLoseType())
                .set(QuoteOpportunities::getStatusBeforeLose,lastStatus)
        ...
        switch (quoteLoseType) {
            case WAIT_SALE_QUOTE_1, WAIT_SALE_QUOTE_2 -> ThreadUtil.runInThread(() -> {
                sendQuoteUnable(quote, dto, EmailTypeEnum.SALES_QUOTATION_LOSS_NOTIFICATION.getCode());
            });
            case SALE_REJECTED, SALE_QUOTE, SIGN_PENDING_APPROVAL, SIGN_REJECTED -> ThreadUtil.runInThread(() -> {
                sendQuoteSignUnable(quote, dto, EmailTypeEnum.QUOTATION_SIGNING_LOSS_NOTIFICATION.getCode());
            });
        }
    ...
    public Boolean salesApproval(ApprovalDTO dto) {
        ...
        baseMapper.update(null,Wrappers.<QuoteOpportunities>lambdaUpdate()
                ...
                .set(QuoteOpportunities::getSaleAuditPerson,currentUser.getUserId())
                .eq(QuoteOpportunities::getId,dto.getOpportunitiesId()));
        //修改子表里的审核理由
    ...
    public Boolean signApproval(ApprovalDTO dto) {
        ...
        baseMapper.update(null,Wrappers.<QuoteOpportunities>lambdaUpdate()
                ...
                .set(QuoteOpportunities::getSignAuditPerson,currentUser.getUserId())
                .eq(QuoteOpportunities::getId,dto.getOpportunitiesId()));
        //修改子表里的审核理由
    ...
    private void approvalEmail(String approvalResult, ApprovalDTO approvalDTO, String approvalType) {
        ...
        if (ObjectUtil.isNotEmpty(quoteOpportunities)) {
            ...
            emailModelInfoConvertDTO.setSignProfitRate(DecimalUtil.getStr(approvalDTO.getCostProfitRate(),DecimalUtil.displayScale,true));
    ...
    public Boolean loseApproval(ApprovalDTO dto) {
        ...
        LambdaUpdateWrapper<QuoteOpportunities> updateWrapper = Wrappers.<QuoteOpportunities>lambdaUpdate()
                ...
                .set(QuoteOpportunities::getLoseAuditDesc, dto.getAuditDesc())
                .set(QuoteOpportunities::getLoseAuditLog,"批准")
                .set(QuoteOpportunities::getLoseAuditPerson, currentUser.getUserId())
                ;
        //把这个商机也丢单
    ...
    private String getLastStatusByLoseType(String loseType){
        String result = null;
        if(ObjectUtil.isNotEmpty(loseType)){
            switch (loseType){
                case "1":
                case "2":
                    result = QuoteStatusEnum.WAIT_SALE_QUOTE.getCode();
                    break;
                case "3":
                    result = QuoteStatusEnum.SALE_REJECTED.getCode();
                    break;
                case "4":
                    result = QuoteStatusEnum.SALE_QUOTE.getCode();
                    break;
                case "5":
                    result = QuoteStatusEnum.SIGN_PENDING_APPROVAL.getCode();
                    break;
                case "6":
                    result = QuoteStatusEnum.SIGN_REJECTED.getCode();
                    break;
                default:
                    result = null;
                    break;
     ...
     */
    private void sendQuoteApply(ReQuoteDTO saveDTO) {
        log.info("----------开始发送待更新成本报价邮件----------");
        SendEmailInfoDTO sendEmailInfoDTO = new SendEmailInfoDTO();
        EmailSubjectInfoConvertDTO emailSubjectInfoConvertDTO = new EmailSubjectInfoConvertDTO();
        EmailModelInfoConvertDTO emailModelInfoConvertDTO = new EmailModelInfoConvertDTO();
        List<SysUser> sysUserList = sysUserMapper.selectList(Wrappers.<SysUser>lambdaQuery());
        // 设置收件人
        QuoteOpportunities quoteOpportunities = quoteOpportunitiesMapper.selectById(saveDTO.getId());
        List<SysUser> recipientUserList =  new ArrayList<>(); // 收件人
        ...
        QuoteEmailSetting quoteEmailSetting = quoteEmailSettingMapper.selectOne(Wrappers.<QuoteEmailSetting>lambdaQuery()
                .eq(QuoteEmailSetting::getType, EmailTypeEnum.PENDING_COST_QUOTATION_UPDATE.getCode()));

        if(ObjectUtil.isEmpty(quoteOpportunities)){
            log.error("----------待更新成本报价不存在,无法发送待更新成本报价邮件----------");
            return;
        }
        // 设置收件人
        Arrays.stream(StringUtil.isNotEmpty(quoteOpportunities.getSupportPerson()) ? quoteOpportunities.getSupportPerson().split(",") : new String[]{})
                .forEach(userId -> sysUserList.stream()
                        .filter(sysUser -> sysUser.getCrmUserId().equals(userId))
                        .findFirst()
                        .ifPresent(recipientUserList::add));
        QuoteOpportunitiesParent quoteOpportunitiesParent = parentMapper.selectById(quoteOpportunities.getOpportunitiesParentId());
        // 设置抄送人
        ccUserList.add(sysUserList.stream().filter(sysUser -> sysUser.getUserId().equals(quoteOpportunitiesParent.getSaleId()))
                .findFirst().orElse(null));
        sendEmailInfoDTO.setCopyUser(ccUserList);

        // 收件人设置邮件基本信息
        recipientUserList.forEach(item ->{
            ...
            // 设置模版信息
            emailModelInfoConvertDTO.setReQuoteDesc(saveDTO.getQuoteDesc());
            emailModelInfoConvertDTO.setQuoteSystemLink(systemConfig.getDomainName());

            sendEmailInfoDTO.setEmailModelInfoConvertDTO(emailModelInfoConvertDTO);
            sendEmailInfoDTO.setEmailSubjectInfoConvertDTO(emailSubjectInfoConvertDTO);
            quoteMailUtil.sendEmail(Collections.singletonList(sendEmailInfoDTO), EmailTypeEnum.PENDING_COST_QUOTATION_UPDATE.getCode());
        });
        log.info("----------待更新成本报价邮件发送完毕----------");
     ...
     * @param dto LoseDTO
     * @param emailTypeEnum emailTypeEnum
     */
    private void sendQuoteUnable(QuoteOpportunities quoteOpportunities,
                                 LoseDTO dto,
                                 String emailTypeEnum) {
        log.info("----------开始发送未销售报价丢单邮件----------");
        SendEmailInfoDTO sendEmailInfoDTO = new SendEmailInfoDTO();
        EmailSubjectInfoConvertDTO emailSubjectInfoConvertDTO = new EmailSubjectInfoConvertDTO();
        EmailModelInfoConvertDTO emailModelInfoConvertDTO = new EmailModelInfoConvertDTO();
        List<SysUser> sysUserList = sysUserMapper.selectList(Wrappers.<SysUser>lambdaQuery());
        ...
        List<SysUser> ccUserList =  new ArrayList<>(); // 抄送人
        QuoteEmailSetting quoteEmailSetting = quoteEmailSettingMapper.selectOne(Wrappers.<QuoteEmailSetting>lambdaQuery()
                .eq(QuoteEmailSetting::getType, emailTypeEnum));
        QuoteOpportunitiesParent quoteOpportunitiesParent = parentMapper.selectById(quoteOpportunities.getOpportunitiesParentId());
        QuotePresaleInfo presaleInfo = presaleInfoMapper.selectOne(Wrappers.<QuotePresaleInfo>lambdaQuery()
                .eq(QuotePresaleInfo::getOpportunitiesId,quoteOpportunities.getId())
                .eq(QuotePresaleInfo::getStatus,VersionStatus.ACTIVE.getCode()));


        if(ObjectUtil.isEmpty(quoteOpportunities)){
            log.error("----------成本报价不存在,无法发送未销售报价丢单邮件----------");
            return;
        }
        // 设置收件人
        Arrays.stream(StringUtil.isNotEmpty(quoteEmailSetting.getSender()) ? quoteEmailSetting.getSender().split(",") : new String[]{})
                .forEach(userId -> sysUserList.stream()
        ...
        recipientUserList.forEach(item ->{
            ...
            if(ObjectUtil.isNotEmpty(quoteOpportunitiesParent)){
                ...
                emailSubjectInfoConvertDTO.setCurrentDate(DateUtil.format(new Date(), DatePattern.NORM_DATETIME_FORMAT));
                sysUserList.stream().filter(sysUser -> sysUser.getUserId().equals(quoteOpportunitiesParent.getSaleId())).findFirst().ifPresent(sysUser -> {
                    emailModelInfoConvertDTO.setPreSaleUser(sysUser.getNickName()); // 所属销售
                });
                emailModelInfoConvertDTO.setBusinessSubject(quoteOpportunitiesParent.getName()); // 商机主题
            }
            // 设置模版信息
            emailModelInfoConvertDTO.setPreSaleUserName(quoteOpportunities.getSupportName()); // 所属售前
            emailModelInfoConvertDTO.setCostAssessmentType(QuoteType.getQuoteTypeByCode(quoteOpportunities.getType()).getInfo());  // 成本评估类型
            emailModelInfoConvertDTO.setCostTotal(DecimalUtil.getStr(quoteOpportunities.getAmount(),DecimalUtil.displayScale)); // 成本总额
            emailModelInfoConvertDTO.setLoseReason(dto.getReasonDesc()); // 丢单理由
            emailModelInfoConvertDTO.setQuoteSystemLink(systemConfig.getDomainName()); // 报价系统链接
            if(ObjectUtil.isNotEmpty(presaleInfo)){
                if(QuoteType.INCAPABLE.getCode().equals(quoteOpportunities.getType())){
                    emailModelInfoConvertDTO.setSaleQuote(DecimalUtil.getStr(
                            presaleInfo.getXxUnableQuoteAmount(),DecimalUtil.displayScale
                    )); // 销售对外报价
                    emailModelInfoConvertDTO.setProfitRate("无"); // 项目报价利润率
                }else {
                    emailModelInfoConvertDTO.setSaleQuote(DecimalUtil.getStr(
                            DecimalUtil.addAll(
                                    presaleInfo.getPreExtQuote(),presaleInfo.getDevExtQuote(),presaleInfo.getProdExtQuote(),presaleInfo.getOtherExtQuote(),
                                    presaleInfo.getSelfExtQuote(),presaleInfo.getExtExtQuote(),presaleInfo.getImpExtQuote()
                            ),
                            DecimalUtil.displayScale
                    )); // 销售对外报价
                    emailModelInfoConvertDTO.setProfitRate(
                            DecimalUtil.getStr(presaleInfo.getProjProfitRateExcl(), DecimalUtil.displayScale,true)
                    ); // 项目报价利润率
                }
            }
            sendEmailInfoDTO.setEmailSubjectInfoConvertDTO(emailSubjectInfoConvertDTO);
            sendEmailInfoDTO.setEmailModelInfoConvertDTO(emailModelInfoConvertDTO);
            quoteMailUtil.sendEmail(Collections.singletonList(sendEmailInfoDTO), emailTypeEnum);
        });
        log.info("----------未销售报价丢单邮件发送完毕----------");
    }
     ...
     * @param emailTypeEnum emailTypeEnum
     */
    private void sendQuoteSignUnable(QuoteOpportunities quoteOpportunities,
                                     LoseDTO dto,
                                     String emailTypeEnum) {
        log.info("----------开始发送报价审批未通过、报价单、签约审批未通过丢单邮件----------");
        SendEmailInfoDTO sendEmailInfoDTO = new SendEmailInfoDTO();
        EmailSubjectInfoConvertDTO emailSubjectInfoConvertDTO = new EmailSubjectInfoConvertDTO();
        EmailModelInfoConvertDTO emailModelInfoConvertDTO = new EmailModelInfoConvertDTO();
        List<SysUser> sysUserList = sysUserMapper.selectList(Wrappers.<SysUser>lambdaQuery());
        ...
        QuoteSignInfo signInfo = signInfoMapper.selectOne(Wrappers.<QuoteSignInfo>lambdaQuery()
                .eq(QuoteSignInfo::getOpportunitiesId,quoteOpportunities.getId())
                .eq(QuoteSignInfo::getStatus,VersionStatus.ACTIVE.getCode()));
        ...
        recipientUserList.forEach(item ->{
            ...
            if(ObjectUtil.isNotEmpty(quoteOpportunities.getSaleAuditTime())){
                emailModelInfoConvertDTO.setApprovalResult(quoteOpportunities.getSaleAuditLog()); // 报价审批结果
            }

            if(ObjectUtil.isNotEmpty(signInfo)){
                emailModelInfoConvertDTO.setApplySignAmount(
                        DecimalUtil.getStr(signInfo.getNorthAmount(),DecimalUtil.displayScale)
                ); // 申请签约金额
                emailModelInfoConvertDTO.setSignProfitRate(
                        DecimalUtil.getStr(signInfo.getCostProfitRate(),DecimalUtil.displayScale,true)
                ); // 项目签约利润率
            }
            if(ObjectUtil.isNotEmpty(quoteOpportunities.getSignAuditTime())){
                emailModelInfoConvertDTO.setSignApprovalResult(quoteOpportunities.getSignAuditLog()); // 签约申请审批结果
            }


            emailModelInfoConvertDTO.setLoseReasonDesc(dto.getReasonDesc()); // 丢单原因
            emailModelInfoConvertDTO.setQuoteSystemLink(systemConfig.getDomainName()); // 报价系统链接
            sendEmailInfoDTO.setEmailSubjectInfoConvertDTO(emailSubjectInfoConvertDTO);
            sendEmailInfoDTO.setEmailModelInfoConvertDTO(emailModelInfoConvertDTO);
            quoteMailUtil.sendEmail(Collections.singletonList(sendEmailInfoDTO), emailTypeEnum);
