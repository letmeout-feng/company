...
import com.internal.common.enums.QuoteParentLoseType;
        ...
        try {
            ...
            businessOpportunityStatusDTOList.forEach(item ->{
                ...
                if (ObjectUtil.isNotNull(quoteOpportunitiesParent) && !(quoteOpportunitiesParent.getCrmState().equals(item.getCurrentState())
                    ...
                    if(item.getCurrentState().equals(QuoteParentStatusEnum.DISCARD.getCode())){
                        quoteOpportunitiesParent.setLoseType(QuoteParentLoseType.CRM.getCode());
                        // 获取子表信息
                        quoteOpportunitiesMapper.selectList(Wrappers.<QuoteOpportunities>lambdaQuery()
                                .ne(QuoteOpportunities::getStatus, QuoteStatusEnum.ABANDON.getCode())
                                .eq(QuoteOpportunities::getOpportunitiesParentId,quoteOpportunitiesParent.getId())).stream().findFirst().ifPresent(innerItem ->{
                            innerItem.setStatus(QuoteStatusEnum.LOST.getCode());
                            innerItem.setLoseTime(item.getUpdateTime());
                            innerItem.setReasonDesc(item.getFailureCause());
                            quoteOpportunitiesMapper.updateById(innerItem);
                        });
                    }
