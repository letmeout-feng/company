<template>
  <div class="app-container">
    <header-title :need-border="true" style="margin-bottom: 20px">
      <template slot="title">丢单审批未通过</template>
    </header-title>
    ...
    <el-table v-loading="loading" :data="data" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="丢单审批人" prop="loseAuditPersonName">
        <template v-slot="scope">
          {{ scope.row.loseAuditPersonName || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="丢单审批理由" prop="loseAuditDesc">
        <template v-slot="scope">
...
export default {
  ...
  methods: {
    ...
    async fetchOpportunitiesData(queryParams) {
      this.loading = true
      try {
        const response = await fetchOpportunities('pageLoseReject', queryParams)
    ...
    async exportData() {
      ...
      this.exportToExcel(exportData, '丢单汇总_丢单审批未通过')
      this.loading = false
    }
  }
}
