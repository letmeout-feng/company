<template>
  <div class="app-container">
    <header-title :need-border="true" style="margin-bottom: 20px;">
      <template slot="title">丢单-销售报价未通过</template>
    </header-title>
...
export default {
  ...
  methods: {
    ...
    async fetchOpportunitiesData(queryParams) {
      this.loading = true
      try {
        const response = await fetchOpportunities('listWithLoseSale', queryParams)
    ...
    async exportData() {
      ...
      const exportData = data.map((item) => ({
        ...
        '成本报价': item.amount,
        '对外报价': item.projExtQuoteTotal,
        '整体利润率': this.formatProfitRate(item.projProfitRateExcl)
      }))
      this.exportToExcel(exportData, '丢单汇总_销售报价未通过')
      this.loading = false
    }
  }
}
