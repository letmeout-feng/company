<template>
  <el-dialog
    ...
    :title="isEditing ? readOnly ? '查看软著' : '编辑软著' : '新建软著'"
    destroy-on-close
    @close="closeDialog"
  >
    <!-- 北光科技 -->
    <el-form v-if="activeTab == '0'" ref="form" :model="currentSoft" :rules="formRules" label-width="120px" :disabled="readOnly">
      ...
      <el-form-item prop="fileList">
        ...
        <FileUpload :file-size="10" :limit="1" :file-list-current="currentSoft.fileList" :read-only="readOnly" @input="handleFileChange" />
      </el-form-item>
    </el-form>
    <!-- 欣象科技 -->
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      if (valid) {
        this.currentSoft.fileList = this.fileList
        if (this.currentSoft.fileList.length < 1) {
          this.$message.error('文件数量不能小于1')
          return
        }
