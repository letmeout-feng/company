    ...
    <div v-else>
      <div class="modal-content">
        ...
        <div class="modal-item">
          ...
          <span style="width: 65%;">{{ record.preSaleName ? record.preSaleName : '-' }}</span>
        </div>
        <div class="modal-item">
          <span style="width: 40%;">申请支持内容：</span>
          <span style="width: 55%;">{{ record.applyDetail || '-' }}</span>
        </div>
        <div class="modal-item" v-if="props.isLoss">
          <span style="width: 30%;">丢单时间 ：</span>
          <span style="width: 65%;">{{ record.loseTime ? record.loseTime : '-' }}</span>
        </div>
        <div class="modal-item" v-if="props.isLoss">
          <span style="width: 30%;">丢单原因 ：</span>
          <span style="width: 65%;">{{ record.reasonDesc ? record.reasonDesc : '-' }}</span>
        </div>
        ...
        <div v-if="props.isLoss && record.loseAuditTime !== null" style="width: 100%;">
          <div class="modal-item" style="width: 100%;margin-top: 20px;">
            <span style="width: 20%;">丢单公司审批：</span>
            <span style="width: 80%;">
              <span>{{ record.loseAuditLog ? record.loseAuditLog : '无' }}&nbsp;&nbsp;</span>
              <span>{{ formatDate(record.loseAuditTime) }}</span>
            </span>
          </div>
          <div class="modal-item" style="width: 100%;">
            <span style="width: 10%;">理由：</span>
            <span style="width: 90%;">{{ record.loseAuditDesc ? record.loseAuditDesc : '无' }}</span>
          </div>
        </div>
      </div>
    </div>
