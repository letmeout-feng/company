    ...
    <a-modal :open="isUnqouteModalVisible" :title="modeTitle" @cancel="handleCancel" width="600px">
        <div class="modal-content">
            ...
            <div class="modal-item" v-if="modalMode === 'reapply'">
                <span style="width: 20%;text-align: right;margin-right: 10px;"><span style="color:red">*</span> 售前申请：</span>
                <span style="width: 80%;">
...
watch(() => props.isUnqouteModalVisible, (newVal) => {
    if (newVal && props.modalMode === 'reapply') {
            fetchRoleUsers();
    }
});
// 发射事件
...
const handleLose = async () => {
    ...
    const quoteOpportunitiesUpdateDTO = {
        ...
        projProfitRateExcl: props.opportunity.projProfitRateExcl,
        projExtQuoteTotal: props.opportunity.projExtQuoteTotal
    };
    if (props.isLostParams === '待销售报价') {
        quoteOpportunitiesUpdateDTO.loseType = '1';
    } else if (props.isLostParams === '报价审批未通过') {
        quoteOpportunitiesUpdateDTO.loseType = '3';
    } else if (props.isLostParams === '报价单') {
        quoteOpportunitiesUpdateDTO.loseType = '4';
    } else if (props.isLostParams === '待签约审批') {
        quoteOpportunitiesUpdateDTO.loseType = '5';
    } else if (props.isLostParams === '签约审批未通过') {
        quoteOpportunitiesUpdateDTO.loseType = '6';
    } else if (props.isLostParams === '丢单审批未通过') {
        quoteOpportunitiesUpdateDTO.loseType = props.opportunity.loseType;
    } else {
        quoteOpportunitiesUpdateDTO.loseType = '';
    }
    try {
