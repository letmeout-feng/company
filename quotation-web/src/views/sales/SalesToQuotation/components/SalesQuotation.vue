<template>
    <a-modal :open="isSaleQuoteModelVisible" :title="`销售报价 - ${comTitle}`" style="width:70%" @cancel="handleCancel">
        ...
        <div v-else>
            <div class="opportunity-introduction">
                <div class="modal-content">
                    ...
                    <div class="modal-item" style="width: 23.3%;">
                        <span>报价版本：</span>
                        <span v-if="isQuoteDetail">{{ currentVersion ? currentVersion : '-' }}</span>
                ...
                <div v-if="opportunity.type === 'ROUGH'" class="desc">
                    <div class="quote-description" @click="toggleDescription">
                        <span style="margin-right:10px">粗略报价说明：</span>
                ...
                <div v-if="opportunity.type === 'COST'" class="desc">
                    <div class="quote-description" @click="toggleDescription">
                        <span style="margin-right:10px">详细报价说明：</span>
                        <span class="description" :class="{ 'expanded': isExpanded }">
                            {{ isExpanded ? (quoteDesc ? quoteDesc : '-') : truncatedDescription }}
                        </span>
                        <span class="toggle-button" v-if="quoteDesc?.length > 30">{{ isExpanded ? '收起' : '展开' }}</span>
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <a-input-number
                                            :style="{ border: quotesPresaleInfo.xxAdditionalQuote !== null ? '2px solid #d9d9d9' : '2px solid rgb(235, 158, 158)' }"
                                            :step="1" :controls="false" placeholder="请输入" :disabled="props.isQuoteDetail"
                                            :precision="0" v-model:value="quotesPresaleInfo.xxAdditionalQuote"
                                            @keydown="preventDecimal" />
                                    </td>
                                    <td></td>
                                    <td></td>
        ...
        <DocumentaryModel :open="isDocumentaryModellVisible" @close="handleDocumentClose"
            ...
            :externalProfit="externalProfit" :projExtQuoteTotal="projExtQuoteTotal" />
    </a-modal>
</template>
...
const isExpanded = ref(false);//收起展开
...
const handleQuoteOk = () => {
    if (signType.value === '1' && quoteType.value === 'cost') {
...
const confirmQuote = (closeNote, followUp, loseParam, callback) => {
...
const truncatedDescription = computed(() => {
    ...
    return (quoteDesc.value ? quoteDesc.value : '-');
});
