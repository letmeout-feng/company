    ...
    try {
        if (followUp.value) {
            if (followUp.value && !closeNote.value.trim()) {   
                ...
                // 确认报价
                emit('confirmQuote', closeNote.value,followUp.value)
            }
        } else {
            await new Promise((resolve, reject) => {
                emit('confirmQuote', '',followUp.value,'丢单', (result) => {
                    if (result && result.success) {
                        resolve(); // 父组件操作成功，继续执行
                    } else {
                        reject('确认报价失败'); // 父组件操作失败
                    }
                });
            });
            // 调用丢单接口  
            ...
            const quoteOpportunitiesUpdateDTO = {
                ...
                projProfitRateExcl: props.projProfitRateExcl,
                projExtQuoteTotal: props.projExtQuoteTotal,
                loseType:'2'
            };
