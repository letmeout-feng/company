            ...
            <a-table :columns="visibleColumns" :data-source="data" :pagination="pagination" rowKey="key" :loading="loading"
                ...
                <template #bodyCell="{ column, record, index }">
                    ...
                    <span v-else-if="column.dataIndex === 'actions'" style="text-align: left;">
                        <a-button v-hasPermi="['quote:rejected:details']" type="link" @click="handleDetail(record)"
                            class="nomp">详情</a-button>
                        <a-button v-hasPermi="['quote:rejected:reapply']" type="link"
                            @click="handleUnqoute('reapply', record)" class="nomp">重新申请报价</a-button>
                        <a-button v-hasPermi="['quote:rejected:handleLost']" type="link"
                            @click="handleUnqoute('discard', record)" class="nomp">丢单处理</a-button>
                        <a-button v-hasPermi="['quote:rejected:update']" type="link" @click="updateSalesOffer(record)"
                            class="nomp">更新销售报价</a-button>
                    </span>
                </template>
            </a-table>
        </a-card>
        <GeneratorQuote v-model:open="generateQuoteVisible" :selectedData="selectedRecord"
            @close="generateQuoteVisible = false" @ok="generateQuoteVisible = false"></GeneratorQuote>
        <HandleUnable :open="isUnqouteModalVisible" :opportunity="selectedRecord" @close="handleCommonClose"
            :modalMode="modalMode" :isLostParams="isLostParams" :isUnqouteModalVisible="isUnqouteModalVisible"/>
...
const handleCommonClose = () => {
    console.log('zhixingdiudanlemdhhahdchnajcfjszhdchajbdbsbnnbfc')
