...
watch(() => props.finalDetailData, (newVal) => {
    if (newVal !== null) {
        ...
        valuationDesc.value = props.finalDetailData?.valuationDesc
...
const getRoughCostQuoteDetail = async (id) => {
    try {
        ...
        if (response && response.code === 200) {
            ...
            valuationDesc.value = response.data.valuationDesc
            roughdata.value = [
...
const truncatedDescription = computed(() => {
    ...
    return (valuationDesc.value ?  valuationDesc.value : '-');
});
