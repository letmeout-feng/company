            ...
            <template #bodyCell="{ column, record, index }">
                ...
                <span v-else-if="column.dataIndex === 'actions'" style="text-align: left;">
                    ...
                    <a-button v-hasPermi="['quote:await:update:update']" type="link"
                        @click="updateDetails(record)" class="nomp">更新报价</a-button>
                </span>
...
const handleUpdateModel = (fromChild) => {
    if (fromChild) {
            isCostVisible.value = false
        }
        // 获取所有版本
        const versionParams = {
            latest: false,
            opportunitiesId: selectedRecord.value.id,
        }
        getVersionFunc(versionParams)
        isAwaitingModel.value = true
